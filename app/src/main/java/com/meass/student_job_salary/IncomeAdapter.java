...
public class IncomeAdapter extends RecyclerView.Adapter<IncomeAdapter.myview> {
    ...
    @NonNull
    @Override
    public IncomeAdapter.myview onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.income, parent, false);
        return new IncomeAdapter.myview(view);
    }

    @Override
    public void onBindViewHolder(@NonNull IncomeAdapter.myview holder, final int position) {
        holder.customer_name.setText(data.get(position).getType());
        ...
        holder.logout1.setText(data.get(position).getUsername());
