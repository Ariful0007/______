...
public class FundAdapter extends RecyclerView.Adapter<FundAdapter.myview> {
    ...
    public FundAdapter(List<Income> data) {
        this.data = data;
    }

    @NonNull
    @Override
    public FundAdapter.myview onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.income, parent, false);
        return new FundAdapter.myview(view);
    }

    @Override
    public void onBindViewHolder(@NonNull FundAdapter.myview holder, final int position) {
        holder.customer_name.setText("Fund Receive");
